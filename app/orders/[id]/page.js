...
export default function OrderDetailPage({ params }) {
  ...
  if (loading) {
    return (
      <div className="text-center py-24 text-gray-400">Loading order...</div>
    );
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      {toast && (
        <div
          className={`fixed top-20 right-4 z-50 px-5 py-3 rounded-xl text-sm font-medium shadow-lg ${
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100 text-sm">
          ...
          <div>
            <p className="text-gray-400 text-xs font-medium mb-1">EST. DELIVERY</p>
        ...
        <table className="w-full text-sm">
          ...
          <tfoot>
            <tr className="bg-gray-50 border-t border-gray-200">
              <td colSpan={3} className="px-6 py-4 font-bold text-gray-700">Total</td>
              <td className="px-6 py-4 text-right font-bold text-indigo-700 text-lg">₹{order.totalAmount}</td>
            </tr>
          </tfoot>
        </table>
      </div>
